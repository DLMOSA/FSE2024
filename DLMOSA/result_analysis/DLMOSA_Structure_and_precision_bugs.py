...
if __name__ == '__main__':
    ...
    # TODO 修改为用于保存结果的文件目录
    result_csv = './DLMOSA_graph_and_precision_bugs.csv'
    ...
    for model_structure in model_structrues:
        ...
        # 未触发bug，直接下一轮
        if float(max([diff_tf_torch_max[Round],diff_torch_mindspore_max[Round],diff_tf_mindspore_max[Round]])) < 0.001:
            Round += 1
            continue
    ...
    #
        ...
        # 统计single_branch_bug和multi_branch_bug
        # To be fair in evaluation, the bound for multi-branch & single-branch model is 2 / 3, which is equal to Ramos.
        if ans / total_edge > (2 / 3):
            single_bug_num += 1
        else:
            multi_bug_num += 1
    ...
    print("single_bug_num:"+str(single_bug_num))
    print("multi_bug_num:"+str(multi_bug_num))
#             total += 1
#             if ans == last_layer + 1:
#                 all_count += 1
