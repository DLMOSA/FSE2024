...
def exe_calculate():
    # 数据太少时，计算没意义，因此前100轮均返回1e-9不计算。
    if len(GlobalConfig.judge_matrix)<=100:
    ...
    fps_score = cal_fitness_value(fps_matrix)
    complexity_score = judge_matrix[-1][3]
    error_score = cal_fitness_value(error_matrix)
...
def calFitness(g):
    ...
    tensorflow_elapsed,torch_elapsed,mindspore_elapsed,tensorflow_fps,torch_fps,mindspore_fps,complexity,diff_1_max,diff_2_max,diff_3_max=exe_module()
    # 执行完之后，judge_matrix的最后一行就是本轮的数据，根据judge_matrix可以计算本轮的fitness。
    fitness = exe_calculate()
    return tensorflow_elapsed,torch_elapsed,mindspore_elapsed,tensorflow_fps,torch_fps,mindspore_fps,complexity,diff_1_max,diff_2_max,diff_3_max,fitness
